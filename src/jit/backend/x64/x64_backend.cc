...
/* clang-format off */
#if PLATFORM_WINDOWS
const int x64_arg0_idx = Xbyak_aarch64::Operand::RCX;
const int x64_arg1_idx = Xbyak_aarch64::Operand::RDX;
const int x64_arg2_idx = Xbyak_aarch64::Operand::R8;
const int x64_arg3_idx = Xbyak_aarch64::Operand::R9;
#else
const int x64_arg0_idx = Xbyak_aarch64::Operand::RDI;
const int x64_arg1_idx = Xbyak_aarch64::Operand::RSI;
const int x64_arg2_idx = Xbyak_aarch64::Operand::RDX;
const int x64_arg3_idx = Xbyak_aarch64::Operand::RCX;
#endif

const Xbyak_aarch64::Reg64 arg0(x64_arg0_idx);
const Xbyak_aarch64::Reg64 arg1(x64_arg1_idx);
const Xbyak_aarch64::Reg64 arg2(x64_arg2_idx);
const Xbyak_aarch64::Reg64 arg3(x64_arg3_idx);
const Xbyak_aarch64::Reg64 guestctx(Xbyak_aarch64::Operand::R14);
const Xbyak_aarch64::Reg64 guestmem(Xbyak_aarch64::Operand::R15);

const struct jit_register x64_registers[] = {
    {"rax",   JIT_RESERVED | JIT_CALLER_SAVE | JIT_REG_I64,                (const void *)&Xbyak_aarch64::util::rax},
    {"rcx",   JIT_RESERVED | JIT_CALLER_SAVE | JIT_REG_I64,                (const void *)&Xbyak_aarch64::util::rcx},
    {"rdx",   JIT_RESERVED | JIT_CALLER_SAVE | JIT_REG_I64,                (const void *)&Xbyak_aarch64::util::rdx},
    {"rbx",   JIT_ALLOCATE | JIT_CALLEE_SAVE | JIT_REG_I64,                (const void *)&Xbyak_aarch64::util::rbx},
    {"rsp",   JIT_RESERVED | JIT_CALLEE_SAVE | JIT_REG_I64,                (const void *)&Xbyak_aarch64::util::rsp},
    {"rbp",   JIT_ALLOCATE | JIT_CALLEE_SAVE | JIT_REG_I64,                (const void *)&Xbyak_aarch64::util::rbp},
#if PLATFORM_WINDOWS
    {"rsi",   JIT_ALLOCATE | JIT_CALLEE_SAVE | JIT_REG_I64,                (const void *)&Xbyak_aarch64::util::rsi},
    {"rdi",   JIT_ALLOCATE | JIT_CALLEE_SAVE | JIT_REG_I64,                (const void *)&Xbyak_aarch64::util::rdi},
    {"r8",    JIT_RESERVED | JIT_CALLER_SAVE | JIT_REG_I64,                (const void *)&Xbyak_aarch64::util::r8},
    {"r9",    JIT_RESERVED | JIT_CALLER_SAVE | JIT_REG_I64,                (const void *)&Xbyak_aarch64::util::r9},
#else
    {"rsi",   JIT_RESERVED | JIT_CALLER_SAVE | JIT_REG_I64,                (const void *)&Xbyak_aarch64::util::rsi},
    {"rdi",   JIT_RESERVED | JIT_CALLER_SAVE | JIT_REG_I64,                (const void *)&Xbyak_aarch64::util::rdi},
    {"r8",    JIT_ALLOCATE | JIT_CALLER_SAVE | JIT_REG_I64,                (const void *)&Xbyak_aarch64::util::r8},
    {"r9",    JIT_ALLOCATE | JIT_CALLER_SAVE | JIT_REG_I64,                (const void *)&Xbyak_aarch64::util::r9},
#endif
    {"r10",   JIT_ALLOCATE | JIT_CALLER_SAVE | JIT_REG_I64,                (const void *)&Xbyak_aarch64::util::r10},
    {"r11",   JIT_ALLOCATE | JIT_CALLER_SAVE | JIT_REG_I64,                (const void *)&Xbyak_aarch64::util::r11},
    {"r12",   JIT_ALLOCATE | JIT_CALLEE_SAVE | JIT_REG_I64,                (const void *)&Xbyak_aarch64::util::r12},
    {"r13",   JIT_ALLOCATE | JIT_CALLEE_SAVE | JIT_REG_I64,                (const void *)&Xbyak_aarch64::util::r13},
    {"r14",   JIT_RESERVED | JIT_CALLEE_SAVE | JIT_REG_I64,                (const void *)&Xbyak_aarch64::util::r14},
    {"r15",   JIT_RESERVED | JIT_CALLEE_SAVE | JIT_REG_I64,                (const void *)&Xbyak_aarch64::util::r15},
#if PLATFORM_WINDOWS
    {"xmm0",  JIT_RESERVED | JIT_CALLER_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm0},
    {"xmm1",  JIT_RESERVED | JIT_CALLER_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm1},
    {"xmm2",  JIT_RESERVED | JIT_CALLER_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm2},
    {"xmm3",  JIT_RESERVED | JIT_CALLER_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm3},
    {"xmm4",  JIT_RESERVED | JIT_CALLER_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm4},
    {"xmm5",  JIT_RESERVED | JIT_CALLER_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm5},
    {"xmm6",  JIT_ALLOCATE | JIT_CALLEE_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm6},
    {"xmm7",  JIT_ALLOCATE | JIT_CALLEE_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm7},
    {"xmm8",  JIT_ALLOCATE | JIT_CALLEE_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm8},
    {"xmm9",  JIT_ALLOCATE | JIT_CALLEE_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm9},
    {"xmm10", JIT_ALLOCATE | JIT_CALLEE_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm10},
    {"xmm11", JIT_ALLOCATE | JIT_CALLEE_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm11},
    {"xmm12", JIT_ALLOCATE | JIT_CALLEE_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm12},
    {"xmm13", JIT_ALLOCATE | JIT_CALLEE_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm13},
    {"xmm14", JIT_ALLOCATE | JIT_CALLEE_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm14},
    {"xmm15", JIT_ALLOCATE | JIT_CALLEE_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm15},
#else
    {"xmm0",  JIT_RESERVED | JIT_CALLER_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm0},
    {"xmm1",  JIT_RESERVED | JIT_CALLER_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm1},
    {"xmm2",  JIT_RESERVED | JIT_CALLER_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm2},
    ...
    {"xmm4",  JIT_RESERVED | JIT_CALLER_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm4},
    {"xmm5",  JIT_RESERVED | JIT_CALLER_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm5},
    {"xmm6",  JIT_ALLOCATE | JIT_CALLER_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm6},
    {"xmm7",  JIT_ALLOCATE | JIT_CALLER_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm7},
    {"xmm8",  JIT_ALLOCATE | JIT_CALLER_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm8},
    {"xmm9",  JIT_ALLOCATE | JIT_CALLER_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm9},
    {"xmm10", JIT_ALLOCATE | JIT_CALLER_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm10},
    {"xmm11", JIT_ALLOCATE | JIT_CALLER_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm11},
    {"xmm12", JIT_ALLOCATE | JIT_CALLER_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm12},
    {"xmm13", JIT_ALLOCATE | JIT_CALLER_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm13},
    {"xmm14", JIT_ALLOCATE | JIT_CALLER_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm14},
    {"xmm15", JIT_ALLOCATE | JIT_CALLER_SAVE | JIT_REG_F64 | JIT_REG_V128, (const void *)&Xbyak_aarch64::util::xmm15},
#endif
};
...
Xbyak_aarch64::Reg x64_backend_reg(struct x64_backend *backend,
  ...
  Xbyak_aarch64::Reg reg = *(const Xbyak_aarch64::Reg *)x64_registers[v->reg].data;
  CHECK(reg.isREG());
...
Xbyak_aarch64::Xmm x64_backend_xmm(struct x64_backend *backend,
                           const struct ir_value *v) {
  CHECK(v->reg >= 0 && v->reg < x64_num_registers);
  Xbyak_aarch64::Xmm xmm = *(const Xbyak_aarch64::Xmm *)x64_registers[v->reg].data;
  CHECK(xmm.isXMM());
  return xmm;
}
...
int x64_backend_push_regs(struct x64_backend *backend, int mask) {
  ...
  for (int i = 0; i < x64_num_registers; i++) {
    ...
    if (r->flags & JIT_REG_I64) {
      Xbyak_aarch64::Reg reg = *(const Xbyak_aarch64::Reg *)r->data;
      CHECK(reg.isREG());
      size += 8;
      e.mov(e.qword[e.rsp - size], reg);
    } else if (r->flags & (JIT_REG_F64 | JIT_REG_V128)) {
      Xbyak_aarch64::Xmm xmm = *(const Xbyak_aarch64::Xmm *)r->data;
      CHECK(xmm.isXMM());
      size += 16;
...
void x64_backend_emit_branch(struct x64_backend *backend, struct ir *ir,
  ...
  if (target) {
    if (ir_is_constant(target)) {
      ...
      Xbyak_aarch64::Reg addr = x64_backend_reg(backend, target);
      e.mov(e.dword[guestctx + guest->offset_pc], addr);
      dispatch_type = 2;
    }
  } else {
  ...
  switch (dispatch_type) {
    case 0:
      e.jmp(block_label, Xbyak_aarch64::CodeGenerator::T_NEAR);
      break;
    case 1:
...
static int x64_backend_assemble_code(struct jit_backend *base, struct ir *ir,
  ...
  try {
    ...
    if (e != Xbyak_aarch64::ERR_CODE_IS_TOO_BIG) {
...
struct jit_backend *x64_backend_create(struct jit_guest *guest, void *code,
  ...
  int have_avx2 = cpu.has(Xbyak_aarch64::util::Cpu::tAVX2);
  int have_sse2 = cpu.has(Xbyak_aarch64::util::Cpu::tSSE2);
  CHECK(have_avx2 || have_sse2, "CPU must support either AVX2 or SSE2");

  backend->codegen = new Xbyak_aarch64::CodeGenerator(code_size, code);
